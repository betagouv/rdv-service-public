  ...
  describe "#covers_date?" do
    ...
    describe "PO weekly wednesdays PM with an end date" do
      let(:plage_ouverture) do
        build(
          :plage_ouverture,
          recurrence: Montrose.every(:week, on: [:wednesday], starts: Date.new(2020, 11, 18), until: Date.new(2020, 12, 9)).to_json,
          first_day: Date.new(2020, 11, 18), # a wednesday
          start_time: Tod::TimeOfDay.new(14),
          end_time: Tod::TimeOfDay.new(18)
        )
    ...
    describe "PO bi-weekly wednesdays PM" do
      let(:plage_ouverture) do
        build(
          :plage_ouverture,
      ...
      context "for a wednesday 1 week later", pending: true do
