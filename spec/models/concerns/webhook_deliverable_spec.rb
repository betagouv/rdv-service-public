RSpec.describe WebhookDeliverable, type: :concern do
  ...
  describe "#send_web_hook" do
    context "when the webhook endpoint is triggered by the model changes" do
      context "on creation" do
        ...
        it "notifies the creation" do
          expect do
            rdv.save
          end.to have_enqueued_job(WebhookBuildAndSendJob).with(record: rdv, action: :created, webhook_endpoint_id: webhook_endpoint.id)
        end
      end
    ...
    context "Special cases for deletion" do
      before do
        webhook_endpoint.update(subscriptions: %w[rdv absence plage_ouverture agent agent_role user user_profile])
      end
