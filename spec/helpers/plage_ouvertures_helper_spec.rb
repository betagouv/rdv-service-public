...
describe PlageOuverturesHelper do
  ...
  describe "#display_recurrence" do
    it "with a weekly recurrence" do
      ...
      expect(display_recurrence(plage_ouverture)).to eq(["Toutes les  semaines, le mardi", "de 08:00 à 12:00", "à partir du mardi 11 janvier 2022"])
    end
    ...
    it "with a weekly recurrence on wednesday" do
      ...
      expect(display_recurrence(plage_ouverture)).to eq(["Toutes les  semaines, les mercredis", "de 08:00 à 12:00", "à partir du mardi 11 janvier 2022"])
    end

    it "with a monthly recurrence" do
      plage_ouverture = build(:plage_ouverture, recurrence: Montrose.every(:month, day: { 3 => [2] }))
      expect(display_recurrence(plage_ouverture)).to eq(["Tous les  mois, le 2ème mercredi", "de 08:00 à 12:00", "à partir du mardi 11 janvier 2022"])
    end
  end
  ...
  describe "#plage_ouverture_occurrence_text" do
    it "returns occurrence text" do
      plage_ouverture = build(:plage_ouverture, recurrence: Montrose.every(:week))
      expect(plage_ouverture_occurrence_text(plage_ouverture)).to eq("Toutes les  semaines, le mardi de 08:00 à 12:00 à partir du mardi 11 janvier 2022")
    end

    it "returns" do
      plage_ouverture = build(:plage_ouverture)
      expect(plage_ouverture_occurrence_text(plage_ouverture)).to eq("mardi 11 janvier 2022de 08:00 à 12:00")
    end
  end
