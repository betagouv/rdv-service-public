...
describe PaperTrailHelper do
  describe "#paper_trail_change_value" do
    ...
    it "returns rdv user ids when property user_ids for rdv resource" do
      ...
      expect(helper.paper_trail_change_value("user_ids", [user1.id, user2.id])).to eq("DUPONT Jeanne, LALOU Martine")
    end
    ...
    it "returns rdv agent ids when property agent_ids with rdv ressource" do
      ...
      expect(helper.paper_trail_change_value("agent_ids", [agent1.id, agent2.id])).to eq("ALLO Patricia, LABAT Marco")
    end
