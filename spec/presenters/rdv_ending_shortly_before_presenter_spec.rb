...
describe RdvEndingShortlyBeforePresenter, type: :presenter do
  ...
  describe "#warning_message" do
    ...
    context "same agent (=> in scope)" do
      ...
      it { is_expected.to match(%r{Vous avez <a .*>un RDV</a> finissant à 08h30 avec FORMAN Milos, vous allez laisser un trou de 30 minutes dans votre agenda}) }
    end
    ...
    context "rdv from other agent but still in scope" do
      ...
      it { is_expected.to match(%r{JOAO Maya a <a .*>un RDV</a> finissant à 08h30 avec FORMAN Milos, vous allez laisser un trou de 30 minutes dans son agenda}) }
    end
    ...
    context "rdv from other agent and not in scope" do
      ...
      it {
        expect(subject).to eq "JOAO Maya a un RDV finissant à 08h30, vous allez laisser un trou de 30 minutes dans son agenda "\
