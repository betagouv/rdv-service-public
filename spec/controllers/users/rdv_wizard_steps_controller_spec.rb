RSpec.describe Users::RdvWizardStepsController, type: :controller do
  describe "#new" do
    ...
    let(:mock_rdv_booking_form) { instance_double(Users::RdvBookingForm, creneau: mock_creneau, rdv: mock_rdv) }

    before { travel_to Date.parse("2020-03-01").in_time_zone + 8.hours }

    context "logged in user" do
      before do
        allow(Users::RdvBookingForm).to \
          receive(:new).with(
            user: user,
            ...
            domain: anything
          ).and_return(mock_rdv_booking_form)
      end
    ...
    context "usager connecté via une invitation" do
      before do
        ...
        allow(Users::RdvBookingForm).to receive(:new).and_return(mock_rdv_booking_form)
      end
