RSpec.describe Admin::RdvsController, type: :controller do
  ...
  describe "POST #export" do
    ...
    it "sends export email" do
      ...
      end.to have_enqueued_job(RdvsExportJob).with(agent: agent, organisation_ids: [organisation.id], options: params.except(:organisation_id, :scoped_organisation_ids).stringify_keys)
    end
  ...
  describe "POST #participations_export" do
    ...
    it "sends export email" do
      ...
      end.to have_enqueued_job(ParticipationsExportJob).with(agent: agent, organisation_ids: [organisation.id], options: params.except(:organisation_id, :scoped_organisation_ids).stringify_keys)
    end
