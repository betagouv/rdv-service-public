...
describe "User can be invited" do
  ...
  describe "when no motifs found through geo search" do
    ...
    let!(:second_motif) do
      create(:motif, name: "RSA orientation telephone", bookable_publicly: true, organisation: organisation2, service: agent.service)
    end
    let!(:second_plage_ouverture) { create(:plage_ouverture, motifs: [second_motif], organisation: organisation2) }
    let!(:collectif_motif) do
      create(:motif, name: "RSA orientation collectif", collectif: true, bookable_publicly: true, organisation: organisation, service: agent.service)
    end
