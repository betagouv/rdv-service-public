RSpec.describe "Agent can create a Rdv with wizard" do
  ...
  describe "create a RDV with an existing lieu" do
    it "works", js: true do
      ...
      expect(page).to have_current_path(admin_organisation_planning_agenda_path(organisation, agent_id: agent, date: rdv.starts_at.to_date, selected_event_id: rdv.id))
      expect(page).to have_content("Le rendez-vous a été créé.")
