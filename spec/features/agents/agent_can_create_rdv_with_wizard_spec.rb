describe "Agent can create a Rdv with wizard" do
  ...
  let!(:agent) { create(:agent, first_name: "Alain", services: [service], basic_role_in_organisations: [organisation]) }
  let!(:agent2) { create(:agent, first_name: "Robert", services: [service], basic_role_in_organisations: [organisation]) }
