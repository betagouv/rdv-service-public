RSpec.describe "un agent peut définir un temps de pause entre chaque RDV" do
  before { travel_to(Time.zone.parse("2026-05-29 08:00")) }

  describe "configuration de du temps de pause" do
    ...
    it "fonctionne" do
      ...
      select("10 min.", from: "Temps de pause après chaque rendez-vous")
  ...
  describe "prise de RDV par un agent (classique)" do
    ...
    it "respecte le temps de pause dans le calcul de créneaux et enregistre le temps de pause dans le RDV créé" do
      login_as(agent, scope: :agent)
