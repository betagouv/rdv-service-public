RSpec.describe "un agent peut définir un temps de battement entre chaque RDV" do
  before { travel_to(Time.zone.parse("2026-05-29 08:00")) }

  describe "configuration de du temps de battement" do
    ...
    it "fonctionne" do
      ...
      select("10 min.", from: "Temps de battement après chaque rendez-vous")
  ...
  describe "prise de RDV par un usager" do
    ...
    it "respecte le temps de battement dans le calcul de créneaux et enregistre le temps de battement dans le RDV créé" do
      ...
      click_on motif.name
      expect(page.all("a[autofocus]").map(&:text)).to eq(["09:00"])
