RSpec.describe "Un agent basique peut voir la configuration de l'organisation" do
  ...
  specify do
    ...
    expect(page).to have_link("Informations de l'organisation")
    ...
    within("tr", text: agent_admin.last_name.upcase) do
      ...
      expect(page).to have_link("Contacter", href: "mailto:#{agent_admin.email}")
    end
    within("tr", text: agent_basic.last_name.upcase) do
      expect(page).not_to have_link("Contacter")
    end
