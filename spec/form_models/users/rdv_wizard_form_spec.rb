RSpec.describe Users::RdvWizardForm do
  ...
  describe "#new" do
    ...
    it "works" do
      ...
      rdv_wizard_form = described_class.new(user:, attributes:, domain:)
      expect(rdv_wizard_form.rdv.user_ids).to eq [user_for_rdv.id]
      expect(rdv_wizard_form.creneau).to eq returned_creneau
    end
    ...
    it "avec un duration_in_min différent de celui du motif" do
      ...
      rdv_wizard_form = described_class.new(user:, attributes: attributes.merge(duration: 60), domain:)
      expect(rdv_wizard_form.rdv.user_ids).to eq [user_for_rdv.id]
      expect(rdv_wizard_form.creneau).to eq returned_creneau
    end
  end
    ...
    context "when the motif is by phone" do
      ...
      context "when the phone number is blank" do
        ...
        it "return false with a rdv by_phone and user without phone" do
          rdv_wizard_form = described_class.new(user:, attributes:, domain:)
          rdv_wizard_form.valid?
          expect(rdv_wizard_form.errors.full_messages.join(", ")).to eq("Le numéro de téléphone est obligatoire car le RDV aura lieu par téléphone")
        end
      end
    end
