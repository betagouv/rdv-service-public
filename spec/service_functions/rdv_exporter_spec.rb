  ...
  describe "#row_array_from rdv" do
    describe "année" do
      it "return year of rdv creation" do
        ...
        expect(RdvExporter.row_array_from(rdv)[0]).to eq(2020)
      end
    end
    ...
    describe "date prise de RDV" do
      it "return year of rdv creation" do
        rdv = build(:rdv, created_at: Time.zone.parse("2020-03-23 12h54"))
        expect(RdvExporter.row_array_from(rdv)[1]).to eq("23/03/2020")
      end
    end
    ...
    describe "heure de prise de RDV" do
      it "return year of rdv creation" do
        rdv = build(:rdv, created_at: Time.zone.parse("2020-03-23 12h54"))
        expect(RdvExporter.row_array_from(rdv)[2]).to eq("12h54")
      end
    end
    ...
    describe "origine" do
      it "return « Créé par un agent » when rdv created by an agent" do
        rdv = build(:rdv)
        expect(RdvExporter.row_array_from(rdv)[3]).to eq("Créé par un agent")
      end

      it "return « RDV pris sur internet » when rdv taken by user" do
        rdv = build(:rdv, created_by: Rdv.created_bies[:user])
        expect(RdvExporter.row_array_from(rdv)[3]).to eq("RDV Pris sur internet")
      end
    ...
    describe "date" do
      it "return rdv Starts_at date, formatted with default" do
        ...
        expect(RdvExporter.row_array_from(rdv)[4]).to eq("07/12/2020")
      end
    end
    ...
    describe "heure rdv" do
      it "return rdv Starts_at time, formatted with time_only" do
        rdv = build(:rdv, starts_at: Time.zone.parse("2020-12-07 12h50"))
        expect(RdvExporter.row_array_from(rdv)[5]).to eq("12h50")
      end
    end
    ...
    describe "service" do
      it "return rdv motif's service name" do
        ...
        expect(RdvExporter.row_array_from(rdv)[6]).to eq("PMI")
      end
    end
    ...
    describe "motif" do
      it "return rdv's motif name" do
        ...
        expect(RdvExporter.row_array_from(rdv)[7]).to eq("Consultation")
      end
    end
    ...
    describe "contexte" do
      it "return rdv's contexte" do
        ...
        expect(RdvExporter.row_array_from(rdv)[8]).to eq("en urgence")
      end
    end
    ...
    describe "statut" do
      it "return rdv statut" do
        ...
        expect(RdvExporter.row_array_from(rdv)[9]).to eq("À renseigner")
      end
    end
    ...
    describe "lieu" do
      it "return « par Téléphone » when rdv by phone" do
        ...
        expect(RdvExporter.row_array_from(rdv)[10]).to eq("Par téléphone")
      end
      ...
      it "return « [domicile] adresse of user » when rdv is at home" do
        ...
        expect(RdvExporter.row_array_from(rdv)[10]).to eq("À domicile")
      end
      ...
      it "return « lieu name and adresse » when rdv in place" do
        ...
        expect(RdvExporter.row_array_from(rdv)[10]).to eq("Centre ville (3 place de la république 56700 Hennebont)")
      end
    end
    ...
    describe "professionnel.le(s)" do
      it "return all agent's names" do
        ...
        expect(RdvExporter.row_array_from(rdv)[11]).to eq("Karima CHARNI, Caroline DUPUIS")
      end
    end
    ...
    describe "usager(s)" do
      it "return all user's names" do
        ...
        expect(RdvExporter.row_array_from(rdv)[12]).to eq("Véronique DIALO, Ayoub PAUL")
      end
    end
