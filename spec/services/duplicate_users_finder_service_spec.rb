RSpec.describe DuplicateUsersFinderService, type: :service do
  describe ".perform" do
    subject(:results) { described_class.new(user: candidate_user, within_territory: territory).perform }
    ...
    describe "when the candidate user is already persisted, it should not match itself" do
      let(:candidate_user) { build(:user, first_name: "Mathieu", last_name: "Lapin", birth_date: "21/10/2000", organisations: [organisation]) }
    ...
    describe "finding by identity (first name + last name + birth date)" do
      context "when there is another user within the territory but with different names" do
        let(:candidate_user) { build(:user, first_name: "Mathieu", last_name: "Lapin") }
        let!(:user_with_different_name) { create(:user, first_name: "Mireille", last_name: "Chasseur", organisations: [organisation]) }
      ...
      context "when there is an homonym (names AND birth date)" do
        let(:candidate_user) { build(:user, first_name: "Mathieu", last_name: "Lapin", birth_date: "21/10/2000") }
        let!(:homonym_user) { create(:user, first_name: "Mathieu", last_name: "Lapin", birth_date: "21/10/2000", organisations: [organisation]) }
    ...
    describe "finding by phone number" do
      context "when there is another user within the territory but with different phone number" do
        ...
        let!(:user_with_different_phone_number) { create(:user, phone_number: "0611111111", organisations: [organisation]) }
      ...
      context "when there is a user with the same phone number" do
        let(:candidate_user) { build(:user, phone_number: "0658032518") }
        let!(:user_with_same_phone_number) { create(:user, phone_number: "0658032518", organisations: [organisation]) }
    ...
    describe "finding by email" do
      context "when there is another user within the territory with different email" do
        ...
        let!(:user_with_different_email) { create(:user, email: "autre@autre.com", organisations: [organisation]) }
      ...
      context "when there is another user within the territory with the same email" do
        let(:candidate_user) { build(:user, email: "candidat@exemple.fr") }
        let!(:user_with_same_email) { create(:user, email: "candidat@exemple.fr", organisations: [organisation]) }
    ...
    describe "finding multiple users with multiple methods" do
      let!(:user_with_matching_identity) { create(:user, first_name: "Mathieu", last_name: "Lapin", birth_date: "21/10/2000", organisations: [organisation]) }
      let!(:user_with_matching_phone) { create(:user, phone_number: "0658032518", organisations: [organisation]) }
      let!(:user_with_matching_email) { create(:user, email: "candidat@exemple.fr", organisations: [organisation]) }
  ...
  describe ".find_duplicate_based_on_names_and_phone" do
    subject { described_class.find_duplicate_based_on_names_and_phone(user: candidate_user, within_territory: territory) }
    ...
    context "there is a duplicate" do
      let!(:duplicated_user) { create(:user, first_name: "Mathieu", last_name: "Lapin", phone_number: "0658032518", organisations: [organisation]) }
    ...
    context "there is a duplicate with accent and whitespaces" do
      let!(:duplicated_user) { create(:user, first_name: "Mathiéu  ", last_name: " Lapïn ", phone_number: "0658032518", organisations: [organisation]) }
