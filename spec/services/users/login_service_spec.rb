RSpec.describe Users::LoginService, type: :service do
  let(:controller) { instance_double(ApplicationController, sign_in: true) }
  let!(:user) { create(:user, email: "test@example.com") }

  context "code valide et utilisable" do
    let!(:login_code) { create(:login_code, email: "test@example.com", code: "123456") }
    let(:service) { described_class.new(email: "test@example.com", code: "123456", controller:) }
  ...
  context "le code saisi n'existe pas" do
    let(:service) { described_class.new(email: "test@example.com", code: "999999", controller:) }
  ...
  context "mauvais le code saisi mais un code utilisable existe" do
    let!(:login_code) { create(:login_code, email: "test@example.com", code: "123456") }
    let(:service) { described_class.new(email: "test@example.com", code: "000000", controller:) }
