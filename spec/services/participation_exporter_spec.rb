RSpec.describe ParticipationExporter, type: :service do
  describe "#xls_string_from_rdvs_rows" do
    it "return export with header" do
      ...
      expect(header_row).to contain_exactly("usager", "rdv_id", "année", "date prise rdv", "heure prise rdv", "origine", "date rdv", "heure rdv", "service", "motif", "contexte", "statut", "lieu",
                                            "professionnel.le(s)", "commune du responsable", "usager mineur ?", "résultat des notifications", "Organisation", "date naissance", "code postal du responsable", "créé par", "email(s) professionnel.le(s)")

      expect(first_data_row).to contain_exactly("Gaston BIDON", rdv.id, 2023, "01/01/2023", "12h50", "Créé par un agent", "07/04/2023", "14h30", "PMI", "Consultation", "des infos sur le rdv",
                                                "À renseigner", "MDS Paris Nord (21 rue des Ardennes, Paris, 75019)", "Francis FACTICE", nil, "non", nil, "MDS Paris", "01/01/2000", nil, "Dans le cadre du RGPD, cette information n'est plus conservée au delà d'un an.", "agent@mail.com")
    end
  end
