RSpec.describe ZammadCustomer do
  describe "#user_or_agent_builder" do
    subject { described_class.user_or_agent_builder(**args).attributes }

    context "usager trouvé par email" do
      let!(:user) { create(:user, email: "soukalina@gmail.com") }
      let(:args) { { email: "soukalina@gmail.com", phone_number: nil } }
    ...
    context "agent trouvé par email" do
      ...
      let(:args) { { email: "agent@example.com", phone_number: nil } }
    ...
    context "usager trouvé par numéro de téléphone" do
      ...
      let(:args) { { phone_number: "0612345678", email: nil } }
    ...
    context "aucun usager ni agent trouvé" do
      let(:args) { { email: "unknown@example.com", phone_number: "013456789" } }
    ...
    context "la correspondance par email prévaut sur le téléphone" do
      ...
      let(:args) { { email: "test@example.com", phone_number: "0612345678" } }
    ...
    context "usager trouvé par numéro de téléphone formatté" do
      ...
      let(:args) { { phone_number: "06 12 34 56 78", email: nil } }
    ...
    context "plusieurs usagers trouvés via le numéro de téléphone non formatté" do
      ...
      let(:args) { { phone_number: "01 23 44", email: nil } }
    ...
    context "usager non trouvé si numéro trop court" do
      ...
      let(:args) { { phone_number: "123", email: nil } }
  ...
  describe "instanciation directe des builders" do
    context "agent" do
      subject { ZammadCustomer::AgentBuilder.new(record: agent).attributes }
    ...
    context "usager" do
      subject { ZammadCustomer::UserBuilder.new(record: user).attributes }

      let!(:user) { create(:user, email: "directuser@example.com") }
