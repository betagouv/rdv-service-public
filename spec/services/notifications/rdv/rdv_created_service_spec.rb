...
describe Notifications::Rdv::RdvCreatedService, type: :service do
  ...
  context "starts today or tomorrow" do
    ...
    it "triggers sending mails to both user and agents" do
      allow(Users::RdvMailer).to receive(:rdv_created).with(rdv, user1).and_return(instance_double(ActionMailer::MessageDelivery, deliver_later: nil))
      allow(Users::RdvMailer).to receive(:rdv_created).with(rdv, user2).and_return(instance_double(ActionMailer::MessageDelivery, deliver_later: nil))
      allow(Agents::RdvMailer).to receive(:rdv_created).with(rdv, agent1).and_return(instance_double(ActionMailer::MessageDelivery, deliver_later: nil))
      allow(Agents::RdvMailer).to receive(:rdv_created).with(rdv, agent2).and_return(instance_double(ActionMailer::MessageDelivery, deliver_later: nil))
      subject
    end
  end
  ...
  context "when rdv is for a relative" do
    ...
    it "calls RdvMailer to send email to responsible" do
      allow(Users::RdvMailer).to receive(:rdv_created).with(rdv, responsible).and_return(instance_double(ActionMailer::MessageDelivery, deliver_later: nil))
      subject
    end
  end
end
