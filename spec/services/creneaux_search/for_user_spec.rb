RSpec.describe CreneauxSearch::ForUser, type: :service do
  ...
  it "call builder without special options" do
    ...
    allow(CreneauxSearch::Calculator).to receive(:available_slots).with(motif:, lieu:, date_range:, agents: []).and_return([creneau_double])
  ...
  it "call with referent as filter when follow_up motif" do
    ...
    allow(CreneauxSearch::Calculator).to receive(:available_slots).with(motif:, lieu:, date_range:, agents: [agent]).and_return([creneau_double])
    creneaux = described_class.new(user: user, motif: motif, lieu: lieu, date_range: date_range).creneaux
    expect(creneaux).to eq [creneau_double]
  end
  ...
  context "with geo search" do
    ...
    it "some attributed agents" do
      ...
      allow(CreneauxSearch::Calculator).to receive(:available_slots).with(motif:, lieu:, date_range:, agents: contain_exactly(agent1, agent2)).and_return([creneau_double])
      creneaux = described_class.new(user: user, motif: motif, lieu: lieu, date_range: date_range, geo_search: mock_geo_search).creneaux
      expect(creneaux).to eq [creneau_double]
    end
  end
