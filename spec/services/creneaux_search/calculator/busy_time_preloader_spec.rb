RSpec.describe CreneauxSearch::Calculator::BusyTimePreloader, type: :service do
  ...
  describe "request to fetch rdvs" do
    it "est optimisée pour utiliser l'index 'calculator_index'. Décommentez le test suivant si celui-ci échoue." do
      ...
      index = ActiveRecord::Base.connection.indexes("agents_rdvs").find { |i| i.name == "calculator_index" }

      expect(index.columns).to eq "agent_id, tsrange(calculator_rdv_starts_at, calculator_rdv_ends_at, '[)'::text)" # les tsrange semblent être plus rapide pour les comparaisons d'intervalle de temps
      expect(index.include).to eq %w[calculator_rdv_starts_at calculator_rdv_ends_at] # Pour avoir un 'covering index'
      expect(index.where).to eq "calculator_rdv_not_cancelled_and_in_the_future"
    end
