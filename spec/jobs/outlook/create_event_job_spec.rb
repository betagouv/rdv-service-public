...
RSpec.describe Outlook::CreateEventJob, type: :job do
  ...
  let(:rdv) { create(:rdv, id: 20, motif: motif, organisation: organisation, starts_at: Time.zone.parse("2023-01-01 11h00"), duration_in_min: 30)}
  let(:agent) { create(:agent) }
  let(:agents_rdv) { create(:agents_rdv, id: 12, rdv: rdv, agent: agent) }
  ...
  context "when the event cannot be created" do
    before do
      ...
      stub_request(:post, "https://graph.microsoft.com/v1.0/me/Events").
         ...
         to_return(status: 200, body: { error: { code: "TerribleError", message: "Quelle terrible erreur" }}.to_json, headers: {})
