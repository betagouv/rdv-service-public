RSpec.describe CronJob::DestroyOldVersions do
  ...
  context "for a table with no personal information" do
    ...
    it "keeps the records for a very long time, but deletes the personal information in the version" do
      ...
      expect(old_motif.versions).to be_present

      expect(recent_motif.versions.first.whodunnit).to be_present
      expect(old_motif.versions.first.whodunnit).to be_blank
    end
  end
end
