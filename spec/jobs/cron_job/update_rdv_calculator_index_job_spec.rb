RSpec.describe CronJob::UpdateRdvCalculatorIndexJob do
  ...
  before do
    ...
    past_rdv.agents_rdvs.first.update(calculator_rdv_not_cancelled_and_in_the_future: true)
  end

  it "updates the AgentsRdv in the past, but not those in the future" do
    expect do
      described_class.new.perform
    end.to change { past_rdv.agents_rdvs.first.reload.calculator_rdv_not_cancelled_and_in_the_future }.from(true).to(false)

    expect(future_rdv.agents_rdvs.first.reload.calculator_rdv_not_cancelled_and_in_the_future).to be_truthy
  end
end
