RSpec.describe UpdatePlageOuverturesExpirationsJob, type: :job do
  subject do
    UpdatePlageOuverturesExpirationsJob.perform_now
  end

  context "when plage ouverture are expired" do
    let!(:plage_ouverture_reguliere) { create(:plage_ouverture, recurrence: Montrose.every(:week, until: Time.now).to_json) }
