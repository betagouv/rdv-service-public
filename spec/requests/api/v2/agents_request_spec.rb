...
describe "api/v2/agents requests", type: :request do
  describe "GET api/v2/agents" do
    subject { get api_v2_agents_path(params), headers: api_auth_headers_for_agent(agent) }
    ...
    context "some agents" do
      ...
      context "policy scoped agents" do
        ...
        it "returns all agents of available organisations" do
          ...
          expect(parsed_response_body.pluck("id")).to match_array([agent.id, agent2.id])
        end
      end
      ...
      context "filtered on organisation" do
        ...
        it "only includes specified organisation" do
          subject
          expect(response.status).to eq(200)
          expect(parsed_response_body.pluck("id")).to match_array([agent.id])
        end
      end
    end
  end
