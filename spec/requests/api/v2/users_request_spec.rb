...
describe "api/v2/users requests", type: :request do
  ...
  describe "GET api/v2/users/:id" do
    context "authorized user ID" do
      ...
      it "works" do
        get api_v2_user_path(user), headers: api_auth_headers_for_agent(agent)
        expect(response.status).to eq(200)
        expect(parsed_response_body).to be_present
        expect(parsed_response_body["id"]).to eq(user.id)
        expect(parsed_response_body["first_name"]).to eq("Jean")
        expect(parsed_response_body["last_name"]).to eq("JACQUES")
        expect(parsed_response_body["user_profiles"]).to be_present
        expect(parsed_response_body["user_profiles"].size).to eq 1
        expect(parsed_response_body["user_profiles"][0]["organisation"]["id"]).to eq(organisation.id)
      end
    end
  ...
  describe "POST api/v2/users" do
    context "valid & minimal params" do
      it "works" do
        ...
        expect(parsed_response_body).to be_present
        expect(parsed_response_body["id"]).to be_present
  ...
  describe "GET api/v2/users" do
    context "multiple organisations" do
      ...
      it "returns policy scoped users" do
        get api_v2_users_path, headers: api_auth_headers_for_agent(agent)
        expect(response.status).to eq(200)
        expect(parsed_response_body.pluck("id")).to contain_exactly(user.id, user2.id)
      end
    end
    ...
    context "when a list of ids is passed" do
      ...
      it "returns the specified user list" do
        ...
        expect(response.status).to eq(200)
        expect(parsed_response_body.pluck("id")).to contain_exactly(user1.id)
      end
    end
  end

  describe "GET api/v2/organisations/:id/users" do
    context "when the agent does not belong to the organisation" do
      ...
      it "does not return the users list" do
        get api_v2_organisation_users_path(other_orga), headers: api_auth_headers_for_agent(agent)
        expect(response.status).to eq(200)
    ...
    context "when the agent belongs to multiple organisations" do
      ...
      it "returns the organisation users only" do
        get api_v2_organisation_users_path(organisation), headers: api_auth_headers_for_agent(agent)
        expect(response.status).to eq(200)
        expect(parsed_response_body.pluck("id")).to contain_exactly(user1.id)
      end
    end
  ...
  describe "GET api/v2/users/:id/invite" do
    context "Existing user with email" do
      ...
      it "works" do
        get invite_api_v2_user_path(user), headers: api_auth_headers_for_agent(agent), as: :json
        expect(response.status).to eq(200)
    ...
    context "unauthorized user ID" do
      ...
      it "works" do
        get invite_api_v2_user_path(user), headers: api_auth_headers_for_agent(agent)
        expect(response.status).to eq(403)
        expect(parsed_response_body["errors"]).to be_present
      end
    end
  ...
  describe "POST api/v2/users/:id/invite" do
    context "Existing user with email" do
      let!(:user) { create(:user, first_name: "Jean", last_name: "JACQUES", organisations: [organisation], email: "jean@jacques.fr") }

      it "works" do
        post invite_api_v2_user_path(user), headers: api_auth_headers_for_agent(agent), as: :json
        expect(response.status).to eq(200)
        expect(parsed_response_body["invitation_url"]).to be_present
        expect(parsed_response_body["invitation_token"]).to be_present
        expect(parsed_response_body["invitation_url"]).to start_with("http://www.example.com/users/invitation/accept?invitation_token=")
  ...
  describe "GET api/v2/:organisation_id/users/:user_id" do
    let!(:user) { create(:user, organisations: [organisation]) }

    context "when the user belongs to the org" do
      it "works" do
        get api_v2_organisation_user_path(organisation, user), headers: api_auth_headers_for_agent(agent)
        expect(response.status).to eq(200)
        expect(parsed_response_body["id"]).to eq(user.id)
      end
    end
  ...
  describe "PATCH api/v2/users" do
    let!(:user) { create(:user, first_name: "Jean", last_name: "JACQUES", organisations: [organisation]) }
