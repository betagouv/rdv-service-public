...
describe "api/v2/absences requests", type: :request do
  let!(:organisation) { create(:organisation) }
  let!(:agent) { create(:agent, basic_role_in_organisations: [organisation]) }

  describe "GET api/v2/absences" do
    subject { get api_v2_absences_path(params), headers: api_auth_headers_for_agent(agent) }
  ...
  describe "POST api/v2/absences" do
    subject { post(api_v2_absences_path, params: params, headers: api_auth_headers_for_agent(agent)) }
  ...
  describe "SHOW api/v2/absences/:id" do
    subject { get(api_v2_absence_path(absence.id), headers: api_auth_headers_for_agent(agent)) }
    ...
    context "authorized absence" do
      ...
      it "works" do
        subject
        expect(response.status).to eq(200)
        expect(parsed_response_body["title"]).to eq(absence.title)
      end
    end
  ...
  describe "PUT api/v2/absences/:id" do
    subject { put(api_v2_absence_path(absence.id), params: params, headers: api_auth_headers_for_agent(agent)) }
  ...
  describe "DELETE api/v2/absences/:id" do
    subject { delete(api_v2_absence_path(absence.id), headers: api_auth_headers_for_agent(agent)) }
