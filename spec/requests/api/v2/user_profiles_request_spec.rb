...
describe "api/v2/user_profiles requests", type: :request do
  let!(:territory) { create(:territory) }
  let!(:organisation) { create(:organisation, territory: territory) }
  let!(:user) { create(:user) }
  let!(:agent) { create(:agent, basic_role_in_organisations: [organisation]) }

  describe "POST api/v2/user_profiles" do
    context "valid & minimal params" do
      it "works" do
        count_before = UserProfile.count
        post(
          api_v2_user_profiles_path,
          params: {
        ...
        expect(parsed_response_body["organisation"]["id"]).to eq(organisation.id)
    ...
    context "valid & complete params" do
      it "works" do
        ...
        expect(parsed_response_body["user"]["id"]).to eq(user.id)
        expect(parsed_response_body["organisation"]["id"]).to eq(organisation.id)
        expect(parsed_response_body["logement"]).to eq("heberge")
        expect(parsed_response_body["notes"]).to eq("Très pressé, vite vite")
        expect(user.reload.organisations).to include(organisation)
      end
    end
