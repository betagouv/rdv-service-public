RSpec.describe Agent::WebhookEndpointPolicy do
  ...
  let(:webhook) { create(:webhook_endpoint, :bypassing_host_validation, organisation: organisation) }
  ...
  describe "#resolve?" do
    ...
    context "with an admin agent" do
      ...
      it "allow to see webhook from same territory" do
        webhook = create(:webhook_endpoint, :bypassing_host_validation, organisation: organisation)
