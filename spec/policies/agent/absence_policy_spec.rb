describe Agent::AbsencePolicy, type: :policy do
  ...
  describe "#show?" do
    ...
    context "regular agent, other agent's absence BUT same service" do
      ...
      let!(:other_agent) { create(:agent, basic_role_in_organisations: [organisation], services: [service]) }
    ...
    context "regular agent, other agent's absence, different service" do
      let!(:service) { create(:service) }
      let!(:agent) { create(:agent, basic_role_in_organisations: [organisation], services: [service]) }
      let!(:other_agent) { create(:agent, basic_role_in_organisations: [organisation], services: [create(:service)]) }
      let!(:absence) { create(:absence, agent: other_agent, organisation: organisation) }
    ...
    context "admin agent, other agent's absence, different service" do
      let!(:service) { create(:service) }
      let!(:agent) { create(:agent, admin_role_in_organisations: [organisation], services: [service]) }
      let!(:other_agent) { create(:agent, basic_role_in_organisations: [organisation], services: [create(:service)]) }
      let!(:absence) { create(:absence, agent: other_agent, organisation: organisation) }
...
describe Agent::AbsencePolicy::Scope, type: :policy do
  describe "#resolve?" do
    ...
    context "misc state" do
      ...
      let!(:absence_same_service) { create(:absence, agent: create(:agent, services: [services[0]]), organisation: organisations[1]) }
      let!(:absence_other_service) { create(:absence, agent: create(:agent, services: [services[1]]), organisation: organisations[1]) }
      let!(:absence_other_service_but_admin) { create(:absence, agent: create(:agent, services: [services[1]]), organisation: organisations[2]) }
      let!(:absence_other_orga) { create(:absence, agent: create(:agent, services: [services[0]]), organisation: organisations[3]) }
