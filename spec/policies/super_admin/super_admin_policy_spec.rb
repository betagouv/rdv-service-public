describe SuperAdmin::SuperAdminPolicy, type: :policy do
  subject { described_class }

  let!(:super_admin) { create(:super_admin) }
  let!(:pundit_context) { super_admin }
  let!(:new_super_admin_member) { build(:super_admin) }

  context "Permitted actions for super_admin" do
    it_behaves_like "permit actions", :new_super_admin_member, :index?, :show?, :new?, :create?, :edit?, :update?, :destroy?
  end

  context "permitted actions for support on super_admin role" do
    let!(:pundit_context) { create(:super_admin, :support) }

    it_behaves_like "permit actions", :new_super_admin_member, :index?, :show?, :new?
    it_behaves_like "not permit actions", :new_super_admin_member, :create?, :edit?, :update?, :destroy?
  end

  context "permitted actions for support on support role" do
    let!(:pundit_context) { create(:super_admin, :support) }
    let!(:new_support_member) { build(:super_admin, :support) }

    it_behaves_like "permit actions", :new_support_member, :index?, :show?, :new?, :create?, :edit?, :update?, :destroy?
  end
end
