...
class User::RdvPolicy < ApplicationPolicy
  ...
  def new?
    ...
    (record.collectif? && record.bookable_publicly_or_invited?) || rdv_belongs_to_user_or_relatives?
  end
  ...
  def show?
    return true if record.collectif? && record.bookable_publicly_or_invited? && rdv_belongs_to_user_or_relatives?
  ...
  class Scope < Scope
    ...
    def resolve
      ...
      scope.where(id: my_rdvs_ids).or(Rdv.where(id: scope.collectif.bookable_publicly_or_invited)).distinct
    end
  end
end
