...
class User::RdvPolicy < ApplicationPolicy
  ...
  def new?
    ...
    (record.collectif? && record.bookable_by_everyone_or_invited?) || rdv_belongs_to_user_or_relatives?
  end
  ...
  def show?
    return true if record.collectif? && record.bookable_by_everyone_or_invited? && rdv_belongs_to_user_or_relatives?
  ...
  class Scope < Scope
    ...
    def resolve
      ...
      scope.where(id: my_rdvs_ids).or(Rdv.where(id: scope.collectif.bookable_by_everyone_or_invited)).distinct
    end
  end
end
