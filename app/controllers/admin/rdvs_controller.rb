class Admin::RdvsController < AgentAuthController
  ...
  def update
    ...
    respond_to do |format|
      format.turbo_stream do
        render locals: { rdv: @rdv, agent: @agent, quick_update: params[:quick_update] }
      end
      format.html do
        if @success
  ...
  def set_optional_agent
    @agent = policy_scope(Agent, policy_scope_class: Agent::AgentPolicy::Scope).where(id: params[:agent_id])
  end
