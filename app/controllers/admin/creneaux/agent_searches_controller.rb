class Admin::Creneaux::AgentSearchesController < AgentAuthController
  ...
  def index
    ...
    if (params[:commit].present? || request.format.js?) && motif_selected? && (results_without_lieu? || only_one_lieu?)
      ...
      @motifs = Agent::MotifPolicy::Scope.apply(current_agent, current_organisation.motifs).active
      ...
      @lieux = Agent::LieuPolicy::Scope.apply(current_agent, current_organisation.lieux).enabled
    end
  end
