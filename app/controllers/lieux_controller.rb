class LieuxController < ApplicationController
  ...
  def index
    ...
    unless online_bookings_suspended_because_of_corona?(@departement)
      @lieux.each do |lieu|
        @next_availability_by_lieux[lieu.id] = FindAvailability.perform_with(@motif_name, lieu, Date.today)
      end
    end
  ...
  def show
    ...
    if online_bookings_suspended_because_of_corona?(@departement)
      ...
      @next_availability = @creneaux.empty? ? FindAvailability.perform_with(@motif_name, @lieu, @date_range.end) : nil
    end
