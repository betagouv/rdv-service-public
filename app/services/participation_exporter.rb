module ParticipationExporter
  ...
  def self.row_array_from(participation)
    ...
    [
      ...
      user.birth_date.present? ? I18n.l(user.birth_date, format: "%d/%m/%Y") : "",
      extract_postal_code_from(user.user_to_notify.address),
      rdv.author,
      rdv.agents.map(&:email).join(", "),
      rdv.motif.collectif ? "oui" : "non",
      created_by ? "[#{participation.created_by_type}] #{created_by.full_name_or_email} (id=#{created_by.id}) (email=#{created_by.email})" : "",
    ]
  end
