...
class Notifiers::RdvCancelled < Notifiers::RdvBase
  ...
  def notify_user_by_mail(user)
    ...
    Users::RdvMailer.rdv_cancelled(@rdv.payload(:destroy, user), user, @rdv_tokens_by_user_id[user.id]).deliver_later
  ...
  def notify_user_by_sms(user)
    ...
    Users::RdvSms.rdv_cancelled(@rdv, user, @rdv_tokens_by_user_id[user.id]).deliver_later
