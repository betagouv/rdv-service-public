...
class Lieu < ApplicationRecord
  ...
  scope :with_open_slots_for_motifs, lambda { |motifs|
    enabled.where(id:
                  PlageOuverture.where.not("recurrence IS ? AND first_day < ?", nil, Time.zone.today)
      .joins(:motifs)
      .where(motifs: { id: motifs.pluck(:id) })
      .map(&:lieu_id)
      .uniq + Rdv.select(:lieu_id).collectif.future.where(motif_id: motifs.pluck(:id)).uniq.map(&:lieu_id))
  }
