class Lieu < ApplicationRecord
  ...
  def self.for_motif_and_departement_from_time(motif_name, departement, now)
    ...
    lieux_ids = PlageOuverture.where("first_day < ?", now).joins(:motifs).where(motifs: { id: motifs_ids }).pluck(:lieu_id).uniq
