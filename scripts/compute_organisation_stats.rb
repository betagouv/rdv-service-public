...
puts "soit #{(avec_email[:oriented] + sans_email[:oriented]/targeted_users_count)*100}% d'usagers orientés en moins de 30 jours"
puts "#{avec_email[:oriented]} usagers avec email sur #{avec_email[:targeted]} orientés en moins de 30 jours"
puts "soit #{(avec_email[:oriented]/avec_email[:targeted])*100}% d'usagers avec mail orientés en moins de 30 jours"
puts "#{sans_email[:oriented]} usagers avec email sur #{sans_email[:targeted]} orientés en moins de 30 jours"
puts "soit #{(sans_email[:oriented]/sans_email[:targeted])*100}% d'usagers sans mail orientés en moins de 30 jours"
puts
