...
namespace :prod do
  ...
  task :massive_cancel, [:end_date] => :environment  do |t, args|
    args.with_defaults(:end_date => 15.days.from_now)
    rdvs = Rdv.future.active.status(:unknown).where('DATE(starts_at) < ?', args.end_date.to_date)
    ...
    rdvs.each do |rdv|
      ...
      rdv.users.map(&:user_to_notify).uniq.each do |user|
        RdvMailer.cancel_rdv(rdv, user).deliver_now if user.email.present?
